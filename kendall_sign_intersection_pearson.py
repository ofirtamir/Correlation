    ...
    for model, methods_data in data.items():
        ...
        for m1, m2 in [("SHAP", "Lime"), ("SHAP", "Inherent"), ("Lime", "Inherent")]:
            if m1 in methods_data and m2 in methods_data:
                fset = set(methods_data[m1].keys()) | set(methods_data[m2].keys())
                v1 = np.array([methods_data[m1].get(f, 0) for f in fset])
                v2 = np.array([methods_data[m2].get(f, 0) for f in fset])
                if len(v1) > 1:
